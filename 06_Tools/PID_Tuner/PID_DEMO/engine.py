...
def run_tuning_engine(bridge, config: Dict, current_pid: Dict[str, float],
    ...
    for rnd in range(1, max_rounds + 1):
        ...
        if not result or result.get("p", -1) < 0:
            err = tuner.last_error if hasattr(tuner, 'last_error') else "unknown"
            print(f"  [LLM FAIL] {err}")
            result = build_fallback_suggestion(safe_pid, metrics)
